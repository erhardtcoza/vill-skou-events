...
function dbg(env, ...args) {
  // turn on with: INSERT OR REPLACE INTO site_settings(key,value) VALUES('WA_DEBUG','1');
  env && console && getSetting(env, "WA_DEBUG").then(v => { if (String(v||"") === "1") console.log("[wa]", ...args); }).catch(()=>{});
}
...
async function waFetch(env, path, body) {
  ...
  if (!token) { dbg(env, "no_token"); return { ok: false, status: 0, json: { error: "no_token" } }; }
  try {
...
export async function sendWhatsAppTemplate(env, to, fallbackText = "", lang, name) {
  ...
  if (!phone_id || !msisdn || !name) { dbg(env, "template precheck failed", { phone_id: !!phone_id, msisdn, name }); return false; }
  const body = {
    ...
    template: {
      ...
      components: [{ type: "body", parameters: [{ type: "text", text: String(fallbackText || "") }] }]
    }
  };
...
export async function sendWhatsAppTextIfSession(env, to, text) {
  ...
  const msisdn = normMSISDN(to);
  if (!phone_id || !msisdn || !text) { dbg(env, "text precheck failed", { phone_id: !!phone_id, msisdn, hasText: !!text }); return false; }
  const body = { messaging_product: "whatsapp", to: msisdn, type: "text", text: { preview_url: true, body: String(text || "") } };
  const res = await waFetch(env, `${phone_id}/messages`, body);
  return !!res.ok;
}
...
export default { sendWhatsAppTemplate, sendWhatsAppTextIfSession, sendOrderOnWhatsApp };
