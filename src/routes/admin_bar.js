...
export function mountAdminBar(router){
  ...
  router.add("GET", "/api/admin/bar/items", guard(async (req, env)=>{
    ...
    const main   = u.searchParams.get("main_menu");

    const where = [];
    const args  = [];
    if (q){
      where.push(`(UPPER(name) LIKE UPPER(?${args.length+1}) OR UPPER(category) LIKE UPPER(?${args.length+1}))`);
      args.push(`%${q}%`);
    }
    if (cat){ where.push(`category = ?${args.length+1}`); args.push(cat); }
    if (active!=null){ where.push(`active = ?${args.length+1}`); args.push(i(active)); }
    if (main!=null){ where.push(`main_menu = ?${args.length+1}`); args.push(i(main)); }

    const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";
    const rows = await env.DB.prepare(
      `SELECT id, name, category, variant, unit, size_ml, is_deposit, active, sort_index, price_cents, main_menu
         FROM pos_items
        ${whereSql}
    ...
    if (id){
      await env.DB.prepare(
        `UPDATE pos_items
          ...
          WHERE id=?11`
      ).bind(f.name,f.category,f.variant,f.unit,f.size_ml,f.is_deposit,f.active,f.sort_index,f.price_cents,f.main_menu,id).run();
      ...
      ).bind(f.name,f.category,f.variant,f.unit,f.size_ml,f.is_deposit,f.active,f.sort_index,f.price_cents,f.main_menu).run();
  ...
  router.add("POST", "/api/admin/bar/items/toggle-main", guard(async (req, env)=>{
    let b; try { b = await req.json(); } catch { return bad("Bad JSON"); }
    const id = i(b?.id); const flag = i(b?.main_menu) ? 1 : 0;
    if (!id) return bad("id required");
  ...
  router.add("GET", "/api/admin/bar/wallets", guard(async (req, env)=>{
    ...
    const args  = [];
    if (q){
      where.push(`(id LIKE ?${args.length+1} OR UPPER(name) LIKE UPPER(?${args.length+1}) OR REPLACE(mobile,' ','') LIKE ?${args.length+1})`);
      args.push(`%${q}%`);
    }
    const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";
  ...
  /* ---------------- BAR: Admin UI pages ---------------- */
  router.add("GET", "/admin/bar", guard(async ()=> {
    const html = await (await import("../ui/admin_bar.js")).barRootHTML();
    return new Response(html, { headers: { "content-type": "text/html; charset=utf-8" }});
  }));
  router.add("GET", "/admin/bar/menu", guard(async ()=> {
    const html = await (await import("../ui/admin_bar_menu.js")).barMenuHTML();
    return new Response(html, { headers: { "content-type": "text/html; charset=utf-8" }});
  }));
  router.add("GET", "/admin/bar/wallets", guard(async ()=> {
    const html = await (await import("../ui/admin_bar_wallets.js")).barWalletsHTML();
    return new Response(html, { headers: { "content-type": "text/html; charset=utf-8" }});
  }));
  router.add("GET", "/admin/bar/cashup", guard(async ()=> {
    const html = await (await import("../ui/admin_bar_cashup.js")).barCashupHTML();
    return new Response(html, { headers: { "content-type": "text/html; charset=utf-8" }});
  }));
}
