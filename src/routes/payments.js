...
/** Mode + key helpers (reads Site Settings first; falls back to env) */
function normMode(s) {
  const v = String(s || "").toLowerCase().trim();
  if (["live", "prod", "production"].includes(v)) return "live";
  if (["simulate", "sim", "demo"].includes(v))    return "simulate";
  return "sandbox"; // default
}
async function getYocoSecret(env, mode) {
  const m = normMode(mode);
  if (m === "live") {
    return (await getSetting(env, "YOCO_LIVE_SECRET")) || env.YOCO_LIVE_SECRET || "";
  }
  // sandbox
  return (await getSetting(env, "YOCO_SANDBOX_SECRET")) || env.YOCO_SANDBOX_SECRET || "";
}
...
export function mountPayments(router) {
  ...
  router.add("POST", "/api/payments/yoco/intent", async (req, env) => {
    ...
    // If already paid, just bounce back to thank-you (UI also polls)
    if (String(o.status || "").toLowerCase() === "paid") {
      const thanks = `${base}/thanks/${encodeURIComponent(code)}`;
      return json({ ok: true, redirect_url: thanks, already_paid: true });
    }

    // Which mode?
    const mode = normMode(
      (await getSetting(env, "YOCO_MODE")) ||
      env.YOCO_MODE ||
      "sandbox"
    );

    // Optional dev-only fast path (only when mode === "simulate")
    if (mode === "simulate") {
      const url = `${base}/api/payments/yoco/simulate?code=${encodeURIComponent(code)}${next ? `&next=${encodeURIComponent(next)}` : ""}`;
      return json({ ok: true, redirect_url: url, mode: "simulate" });
    }

    // Build the YOCO request
    const secret = await getYocoSecret(env, mode);
    if (!secret) return bad("Missing Yoco secret key", 500);

    const successUrl = `${base}/thanks/${encodeURIComponent(code)}`;
    const cancelUrl  = `${base}/thanks/${encodeURIComponent(code)}?pay=cancel`;
    const failureUrl = `${base}/thanks/${encodeURIComponent(code)}?pay=failed`;

    const payload = {
      amount: Number(o.total_cents || 0),        // cents
      currency: "ZAR",
      successUrl,
      cancelUrl,
      failureUrl,
      metadata: {
        reference: o.short_code,                 // help webhook map to order
        orderShortCode: o.short_code
      },
    ...
    try {
      ...
      if (!r.ok || !J?.redirectUrl) {
        // surface whatever Yoco returned to help debug
        return json({ ok: false, error: "yoco_error", detail: J }, 502);
      }
      // Hand the hosted checkout URL back to the browser
      return json({ ok: true, redirect_url: J.redirectUrl, mode });
    } catch (e) {
      return json({ ok: false, error: "yoco_network", detail: String(e?.message||e) }, 502);
    }
  });

  /**
   * DEV ONLY: simulator that marks the order paid and redirects back
   * Keep this for emergencies or manual testing.
   */
   ...
   * Configure in the Yoco dashboard.
   */
