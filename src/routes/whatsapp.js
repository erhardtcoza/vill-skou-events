...
async function sendViaService(env, to, payload) {
  // payload: { type:'text', text:'...' }  or  { type:'template', name:'x', language:'af', variables:[...] }
  try {
    const svc = await import("../services/whatsapp.js");
    // Try common signatures without crashing the request if a variant isn’t present.
    if (payload.type === "text") {
      // A) sendWhatsAppText(env, to, text)
      if (typeof svc.sendWhatsAppText === "function") {
        await svc.sendWhatsAppText(env, to, payload.text);
        return true;
      }
      // B) sendWhatsAppText(env, { to, text })
      if (typeof svc.sendWhatsAppText === "function") {
        await svc.sendWhatsAppText(env, { to, text: payload.text });
        return true;
      }
      // C) sendWhatsAppTextIfSession(env, to, text)
      if (typeof svc.sendWhatsAppTextIfSession === "function") {
        await svc.sendWhatsAppTextIfSession(env, to, payload.text);
        return true;
      }
    } else if (payload.type === "template") {
      // A) sendWhatsAppTemplate(env, to, variables, language, name)
      ...
      // B) sendWhatsAppTemplate(env, {to,name,language,variables})
      if (typeof svc.sendWhatsAppTemplate === "function") {
    ...
    // services module missing is fine; we'll fall back
  }
  return false;
}
...
async function sendWA(env, to, payload) {
  // Try your local service first; fall back to Graph API if needed.
  const ok = await sendViaService(env, to, payload);
  if (ok) return null;
...
function extractInbound(valueObj) {
  ...
  // We only need messages array; ignore statuses for now
  const results = [];
...
export function mountWhatsApp(router) {
  ...
  router.add("POST", "/api/whatsapp/webhook", async (req, env) => {
    ...
    for (const e of entries) {
      ...
      for (const ch of changes) {
        ...
        for (const r of arr) {
          if (!r.wa_id) continue;
          // insert, ignore duplicates by wa_id unique
          try {
          ...
          try {
            ...
            // log failures but don't break webhook
            try {
              await env.DB.prepare(
                `INSERT INTO wa_logs (to_msisdn, type, payload, status, created_at)
  ...
  /* ---------------- Admin: inbox list ------------------------- */
  router.add("GET", "/api/admin/whatsapp/inbox", async (req, env) => {
    // guard in admin.js wraps everything with requireRole("admin"), so we can mirror that
    // or you can add an explicit guard here if you prefer.
    // For simplicity we reuse the same pattern as other admin routes: use a guard at router registration layer if needed.
    const u = new URL(req.url);
  ...
  router.add("POST", "/api/admin/whatsapp/inbox/:id/reply", async (req, env, _ctx, { id }) => {
    ...
    try {
      ...
      await env.DB.prepare(
        `UPDATE wa_inbox SET replied_manual=1 WHERE id=?1`
      ).bind(row.id).run();
      await env.DB.prepare(
        `INSERT INTO wa_logs (to_msisdn, type, payload, status, created_at)
