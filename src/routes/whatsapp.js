...
async function sendDirectGraph(env, to, payload) {
  ...
  if (payload.type === "text") {
    ...
    body = {
      ...
      template: {
        ...
        components:
          Array.isArray(payload.variables) && payload.variables.length
            ? [
                {
                  type: "body",
                  parameters: payload.variables.map((t) => ({
                    type: "text",
                    text: String(t)
                  }))
                }
  ...
  const res = await fetch(
    "https://graph.facebook.com/v20.0/" + enc(pnid) + "/messages",
    {
      method: "POST",
...
export async function sendTemplateByKey(env, { template_key, msisdn, data }) {
  ...
  const walletId =
    data?.wallets?.id || data?.wallet_id || "";
  const base =
    (await getSetting(env, "PUBLIC_BASE_URL")) ||
    env.PUBLIC_BASE_URL ||
  ...
  switch (name) {
    case "bar_welcome":
      ...
      break;
    case "bar_topup":
      variables = [R(mvCents), walletURL, R(balCents)];
      break;
    case "bar_purchase":
      variables = [R(mvCents), R(balCents)];
      break;
...
function extractInbound(valueObj) {
  ...
  for (const m of messages) {
    ...
    results.push({
      ...
      to_msisdn:
        valueObj?.metadata?.display_phone_number ||
        valueObj?.metadata?.phone_number_id ||
        null,
      body: text,
      type: t,
...
export function mountWhatsApp(router) {
      ...
      for (const ch of changes) {
        ...
        for (const r of arr) {
          ...
          try {
            ...
            const yes =
              String(enabled).trim() === "1" || /true/i.test(String(enabled));
            if (yes && text) {
              ...
              await env.DB.prepare(
                ...
                .bind(r.from_msisdn, text, nowSec())
                .run();
            }
          } catch {
            try {
  ...
  async function listInbox(req, env) {
    ...
    if (q) {
      clauses.push(
        "(UPPER(from_msisdn) LIKE UPPER(?1) OR UPPER(body) LIKE UPPER(?1))"
      );
      args.push("%" + q + "%");
    }
    ...
    const payload = {
      ok: true,
      items: list.results || [],
      rows: list.results || [], // keep for older UI
  ...
  router.add(
    "POST",
    "/api/admin/whatsapp/inbox/:id/reply",
    async (req, env, _ctx, { id }) => {
      let b;
      try {
        b = await req.json();
      } catch {
      ...
      try {
        ...
        await env.DB.prepare("UPDATE wa_inbox SET replied_manual=1 WHERE id=?1")
          .bind(row.id)
          .run();
        await env.DB.prepare(
          "INSERT INTO wa_logs (to_msisdn, type, payload, status, created_at) VALUES (?1,'manual_reply',?2,'sent',?3)"
        )
          .bind(row.from_msisdn, text, nowSec())
          .run();
        return json({ ok: true });
      } catch (e) {
        await env.DB.prepare(
  ...
  router.add(
    "POST",
    "/api/admin/whatsapp/inbox/:id/delete",
    async (_req, env, _ctx, { id }) => {
  ...
  router.add("POST", "/api/admin/whatsapp/mappings/save", async (req, env) => {
    ...
    await env.DB.prepare(
      ...
      .bind(key, context, JSON.stringify(mapping), nowSec())
      .run();
