...
export function mountAdmin(router) {
  ...
  router.add("GET", "/api/admin/summary", guard(async (_req, env) => {
    ...
    for (const ev of (evQ.results || [])) {
      ...
      sums[ev.id] = {
        total:   Number(tQ && tQ.total  || 0),
        unused:  Number(tQ && tQ.unused || 0),
        inside:  Number(tQ && tQ.inside || 0),
        outside: Number(tQ && tQ.outside|| 0),
        voided:  Number(tQ && tQ.voided || 0),
      };
    }
  ...
  router.add("GET", "/api/admin/settings", guard(async (_req, env) => {
    const wanted = [
      ...
      // Optional OAuth-style fields if you added them
      "YOCO_CLIENT_ID",
      "YOCO_REDIRECT_URI",
  ...
  router.add("POST", "/api/admin/settings/update", guard(async (req, env) => {
    ...
    const updates = b && b.updates && typeof b.updates === "object" ? b.updates : null;
    if (!updates) return bad("updates required");

    for (const rawKey in updates) {
      const dbKey = normalizeInKey(String(rawKey || "").trim());
      const val = updates[rawKey] == null ? "" : String(updates[rawKey]);
  ...
  router.add("GET", "/api/admin/whatsapp/templates", guard(async (_req, env) => {
    const q = await env.DB.prepare(
      ...
      "FROM wa_templates ORDER BY name ASC"
    ).all();
  ...
  router.add("POST", "/api/admin/whatsapp/templates/create", guard(async (req, env) => {
    ...
    const language = String(b && b.language || "").trim() || "en_US";
    const category = String(b && b.category || "").trim() || "TRANSACTIONAL";
    const status = String(b && b.status || "").trim() || "pending";
    ...
    await env.DB.prepare(
      ...
      "VALUES (?1, ?2, ?3, ?4, ?5)"
    ).bind(name, language, status, category, now).run();

    return json({ ok: true });
  }));

  // Placeholder "sync" – no Meta calls yet; returns count for UI
  router.add("POST", "/api/admin/whatsapp/templates/sync", guard(async (_req, env) => {
    const q = await env.DB.prepare(
      "SELECT COUNT(*) AS cnt FROM wa_templates"
    ).all();
    const cnt = (q.results && q.results[0] && q.results[0].cnt) ? Number(q.results[0].cnt) : 0;
    return json({ ok: true, count: cnt });
  }));
  ...
  router.add("GET", "/api/admin/events/:id", guard(async (_req, env, _ctx, p) => {
  ...
  router.add("POST", "/api/admin/events/save", guard(async (req, env) => {
    ...
    const fields = {
      slug: (b && b.slug || "").trim(),
      name: (b && b.name || "").trim(),
      venue: (b && b.venue || "").trim(),
      starts_at: Number(b && b.starts_at || 0),
      ends_at: Number(b && b.ends_at || 0),
      status: (b && b.status || "active").trim(),
      hero_url: (b && b.hero_url) || null,
      poster_url: (b && b.poster_url) || null,
      gallery_urls: (b && b.gallery_urls) || null
    };

    if (id) {
      await env.DB.prepare(
        "UPDATE events SET " +
        "slug=?1, name=?2, venue=?3, starts_at=?4, ends_at=?5, status=?6, " +
  ...
  router.add("GET", "/api/admin/events/:id/ticket-types", guard(async (_req, env, _ctx, p) => {
    const id = Number(p && p.id || 0);
    const q = await env.DB.prepare(
  ...
  router.add("POST", "/api/admin/ticket-types/save", guard(async (req, env) => {
    let b; try { b = await req.json(); } catch { return bad("Bad JSON"); }
    const id       = Number(b && b.id || 0);
    const event_id = Number(b && b.event_id || 0);
    if (!event_id) return bad("event_id required");

    const fields = {
      name: (b && b.name || "").trim(),
      code: (b && b.code || null),
      price_cents: Number(b && b.price_cents || 0),
      capacity: Number(b && b.capacity || 0),
      per_order_limit: Number(b && b.per_order_limit || 10),
      requires_gender: Number(b && b.requires_gender || 0) ? 1 : 0
    };

    if (id) {
      await env.DB.prepare(
        "UPDATE ticket_types SET " +
        "name=?1, code=?2, price_cents=?3, capacity=?4, per_order_limit=?5, requires_gender=?6 " +
  ...
  router.add("GET", "/api/admin/orders/by-code/:code", guard(async (_req, env, _ctx, p) => {
  ...
  router.add("GET", "/api/admin/pos/sessions", guard(async (_req, env) => {
    ...
    const tQ = await env.DB.prepare(
      ...
      "FROM pos_payments GROUP BY session_id"
    ).all();

    const totals = {};
    for (const r of (tQ.results || [])) {
      totals[r.session_id] = {
        cash_cents: Number(r && r.cash_cents || 0),
        card_cents: Number(r && r.card_cents || 0)
      };
    }
    ...
    const out = sessions.map(s => ({
      ...
      cash_cents: totals[s.id] && totals[s.id].cash_cents || 0,
      card_cents: totals[s.id] && totals[s.id].card_cents || 0
    }));
  ...
  router.add("POST", "/api/admin/vendors/save", guard(async (req, env) => {
    ...
    const fields = {
      name: (b && b.name || "").trim(),
      contact_name: (b && b.contact_name || null),
      phone: (b && b.phone || null),
      email: (b && b.email || null),
      stand_number: (b && b.stand_number || null),
      staff_quota: Number(b && b.staff_quota || 0),
      vehicle_quota: Number(b && b.vehicle_quota || 0)
    };
  ...
  // Vendor passes: list
  router.add("GET", "/api/admin/vendor/:id/passes", guard(async (_req, env, _ctx, p) => {
    const id = Number(p && p.id || 0);
  ...
  // Vendor passes: add
  router.add("POST", "/api/admin/vendor/:id/pass/add", guard(async (req, env, _ctx, p) => {
    let b; try { b = await req.json(); } catch { return bad("Bad JSON"); }
    const vendor_id = Number(p && p.id || 0);
    if (!vendor_id) return bad("vendor_id required");

    const type = String(b && b.type || "").trim(); // 'staff' | 'vehicle'
    if (!(type === "staff" || type === "vehicle")) return bad("Invalid type");

    const label = String(b && b.label || "").trim();
    const vehicle_reg = (type === "vehicle") ? (String(b && b.vehicle_reg || "").trim()) : null;
    ...
    return json({ ok: true, qr: qr });
  }));

  // Vendor passes: delete
  router.add("POST", "/api/admin/vendor/:id/pass/delete", guard(async (req, env, _ctx, p) => {
    let b; try { b = await req.json(); } catch { return bad("Bad JSON"); }
    const pid = Number(b && b.pass_id || 0);
    if (!pid) return bad("pass_id required");
    await env.DB.prepare(
      "DELETE FROM vendor_passes WHERE id = ?1 AND vendor_id = ?2"
    ).bind(pid, Number(p && p.id || 0)).run();
    return json({ ok: true });
  }));
  ...
  // Expects /src/services/whatsapp.js exporting sendWhatsAppTemplate(env, to, body, lang)
  router.add("POST", "/api/admin/orders/:code/send-whatsapp", guard(async (req, env, _ctx, p) => {
    let b; try { b = await req.json(); } catch { b = {}; }
    const msisdn = String(b && b.phone || "").trim();
    if (!msisdn) return bad("phone required");

    const c = String(p && p.code || "");
    const o = await env.DB.prepare(
    ...
    try {
      ...
      sendFn = mod && mod.sendWhatsAppTemplate ? mod.sendWhatsAppTemplate : null;
    } catch (_e) {
      return bad("WhatsApp service not available");
    }
    if (!sendFn) return bad("WhatsApp service not available");

    const publicBase = await getSetting(env, "PUBLIC_BASE_URL");
    const link = o.short_code ? (String(publicBase || "") + "/t/" + o.short_code) : String(publicBase || "");
    const body = "Jou kaartjies is gereed.\nBestel nommer: " + o.short_code + "\n" + link;

    try {
      await sendFn(env, msisdn, body, "en_US");
      return json({ ok: true });
    } catch (e) {
      return bad(String(e && e.message || e || "WhatsApp send failed"), 500);
    }
  }));
