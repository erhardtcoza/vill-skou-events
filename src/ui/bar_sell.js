// /src/ui/bar_sell.js
export function barSellHTML() {
  return `<!doctype html><html lang="af"><head>
<meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Bar verkope</title>
<style>
  :root{
    --ink:#0b1320; --muted:#667085; --bg:#f6f8f7; --card:#fff; --accent:#0a7d2b; --danger:#b42318; --border:#e5e7eb;
  }
  *{ box-sizing:border-box }
  body{ margin:0; background:var(--bg); color:var(--ink); font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif }
  .wrap{ max-width:900px; margin:16px auto; padding:0 12px }
  h1{ margin:0 0 12px }
  .card{ background:var(--card); border-radius:14px; box-shadow:0 10px 22px rgba(0,0,0,.07); padding:14px }
  .row{ display:flex; gap:8px; align-items:center; flex-wrap:wrap }
  input{ padding:12px; border:1px solid var(--border); border-radius:12px; font:inherit; background:#fff; width:100% }
  .chip{ display:inline-flex; align-items:center; padding:8px 12px; border:1px solid var(--border); border-radius:999px; background:#f9fafb; cursor:pointer; }
  .chip.active{ background:var(--accent); color:#fff; border-color:var(--accent) }
  .muted{ color:var(--muted) }
  .items{ display:grid; grid-template-columns:repeat(auto-fill, minmax(210px, 1fr)); gap:10px; margin-top:8px }
  .item{ border:1px solid var(--border); border-radius:12px; padding:12px; cursor:pointer; background:#fff }
  .item:hover{ border-color:#cbd5e1 }
  .item .name{ font-weight:800; margin:0 0 6px }
  .item .price{ color:#444 }
  .cart{ min-height:80px; background:#fafafa; border:1px dashed var(--border); border-radius:12px; padding:8px }
  .cart-line{ display:flex; justify-content:space-between; align-items:center; padding:6px 0; border-bottom:1px solid #f1f3f5 }
  .cart-line:last-child{ border-bottom:0 }
  .btn{ padding:11px 14px; border-radius:12px; border:0; cursor:pointer; font-weight:800 }
  .btn.primary{ background:var(--accent); color:#fff }
  .btn.light{ background:#e5e7eb; color:#111 }
  .total{ font-size:18px; font-weight:900 }
  .topbar{ display:grid; grid-template-columns: 1fr auto auto; gap:8px }
  @media (max-width:560px){ .topbar{ grid-template-columns:1fr auto } }
</style>
</head><body>
<div class="wrap">
  <h1>Bar verkope</h1>

  <div class="card" id="panel">
    <!-- Wallet controls -->
    <div class="topbar">
      <input id="wallet" placeholder="Wallet ID of selfoon"/>
      <button id="scanBtn" class="btn light">Scan</button>
      <button id="loadBtn" class="btn light">Laai</button>
    </div>
    <div id="cust" class="muted" style="margin:8px 0 6px">—</div>

    <!-- CART (top only) -->
    <h3 style="margin:8px 0 6px">Mandjie</h3>
    <div id="cart" class="cart"></div>
    <div class="row" style="justify-content:space-between; margin-top:8px">
      <div>
        <span class="muted">Totaal</span> • <span id="total" class="total">R0.00</span>
        <span id="remain" class="muted" style="margin-left:10px"></span>
      </div>
      <div class="row">
        <button id="checkoutBtn" class="btn primary">Klaar</button>
        <button id="clearBtn" class="btn light">Maak skoon</button>
      </div>
    </div>

    <!-- Items -->
    <div id="cats" class="row" style="margin:12px 0 6px"></div>
    <div id="items" class="items"></div>
    <div id="leftMsg" class="muted" style="margin-top:6px"></div>
  </div>
</div>

<script>
/* ------------------ helpers ------------------ */
const $ = id => document.getElementById(id);
const money = c => 'R'+((Number(c)||0)/100).toFixed(2);
const cents = n => Number(n||0)|0;
const digits = s => String(s||'').replace(/\\D+/g,'');

/* ------------------ caches ------------------- */
const ITEMS_KEY = 'BAR_ITEMS_CACHE_V1';
const ITEMS_TTL = 10*60*1000; // 10 min
const WALLETS_LRU_KEY = 'BAR_WALLETS_LRU_V1';
const WALLETS_MAX = 50;

function loadItemsCache(){
  try{
    const j = JSON.parse(localStorage.getItem(ITEMS_KEY)||'{}');
    if (j.ts && Date.now()-j.ts < ITEMS_TTL && Array.isArray(j.items)) return j.items;
  }catch{}
  return null;
}
function saveItemsCache(items){
  try{ localStorage.setItem(ITEMS_KEY, JSON.stringify({ ts: Date.now(), items })); }catch{}
}
function walletsLRULoad(){
  try{ return JSON.parse(localStorage.getItem(WALLETS_LRU_KEY)||'[]')||[]; }catch{ return []; }
}
function walletsLRUSave(list){
  try{ localStorage.setItem(WALLETS_LRU_KEY, JSON.stringify(list.slice(0,WALLETS_MAX))); }catch{}
}
function walletsLRUTouch(w){
  if (!w?.id) return;
  const list = walletsLRULoad().filter(x => x.id !== w.id);
  list.unshift({ id: w.id, name: w.name||'', mobile: w.mobile||'', balance_cents: w.balance_cents|0, version: Number(w.version||0), ts: Date.now() });
  walletsLRUSave(list);
}
function walletFromLRUByInput(raw){
  // match either id or phone digits
  const d = digits(raw);
  const list = walletsLRULoad();
  return list.find(x => x.id === raw || (d && (x.mobile||'').replace(/\\D+/g,'').endsWith(d)));
}

/* ------------------ state ------------------- */
let walletId = null, version = 0, balance = 0;
let allItems = [];     // API payload
let cart = new Map();  // id -> { name, price_cents, qty }
let currentCat = null;

/* ------------------ UI ------------------- */
const afrMap = {
  "Beer":"Bier","Wine":"Wyn","Spirits":"Sterk drank","Soft drink":"Koeldrank",
  "Cider":"Ciders","Shots":"Shooters","Deposit":"Deposit"
};
function uniqueCats(items){
  const set = new Set(items.map(i => String(i.category||'')));
  return Array.from(set).filter(Boolean);
}
function displayName(it){
  const base = it.name || '';
  const v = (it.variant||'').trim();
  return v ? \`\${base} · \${v}\` : base;
}
function renderCats(){
  const cats = uniqueCats(allItems);
  const html = cats.map(c => \`<span class="chip \${c===currentCat?'active':''}" data-cat="\${c}">\${afrMap[c]||c}</span>\`).join('');
  $('cats').innerHTML = html || '<span class="muted">Geen kategorieë</span>';
  $('cats').querySelectorAll('[data-cat]').forEach(el=>{
    el.onclick = ()=> { currentCat = el.getAttribute('data-cat'); renderCats(); renderItems(); };
  });
}
function renderItems(){
  const list = allItems.filter(it => it.category === currentCat);
  $('items').innerHTML = list.map(it => \`
    <div class="item" data-id="\${it.id}">
      <div class="name">\${displayName(it)}</div>
      <div class="price">\${it.price_cents==null ? '' : money(it.price_cents)}</div>
    </div>\`
  ).join('');
  $('leftMsg').textContent = list.length ? '' : 'Geen items in hierdie kategorie.';
  $('items').querySelectorAll('[data-id]').forEach(el=>{
    el.onclick = () => addToCart(Number(el.getAttribute('data-id')));
  });
}
function renderCart(){
  const rows = Array.from(cart.values());
  $('cart').innerHTML = rows.length ? rows.map(r => \`
    <div class="cart-line">
      <div>\${r.qty}× \${r.name}</div>
      <div>\${money(r.qty * (r.price_cents||0))}</div>
    </div>\`
  ).join('') : '<div class="muted">Leeg.</div>';

  const total = rows.reduce((s,r)=> s + r.qty*(r.price_cents||0), 0);
  $('total').textContent = money(total);
  const remain = balance - total;
  $('remain').textContent = balance ? ('Oorblyf: '+money(remain)) : '';
}
function addToCart(id){
  const it = allItems.find(x => Number(x.id)===Number(id));
  if (!it) return;
  const key = String(id);
  const cur = cart.get(key) || { id, name: displayName(it), price_cents: cents(it.price_cents), qty: 0 };
  cur.qty++;
  cart.set(key, cur);
  renderCart();
}
function clearCart(){ cart.clear(); renderCart(); }

/* ------------------ data loads ------------------- */
async function ensureItems(){
  const cached = loadItemsCache();
  if (cached) {
    allItems = cached;
    const cats = uniqueCats(allItems);
    currentCat = currentCat || cats[0] || null;
    renderCats(); renderItems();
    // background refresh
    fetch('/api/items').then(r=>r.json()).then(j=>{
      const items = j?.items||[];
      if (items.length) { allItems = items; saveItemsCache(items); renderCats(); renderItems(); }
    }).catch(()=>{});
    return;
  }
  try{
    const j = await fetch('/api/items').then(r=>r.json());
    allItems = j?.items || [];
    saveItemsCache(allItems);
  }catch(_e){ allItems = []; }
  const cats = uniqueCats(allItems);
  currentCat = cats[0] || null;
  renderCats(); renderItems();
}

async function lookupWallet(raw){
  // instant optimistic display if cached
  const cached = walletFromLRUByInput(raw);
  if (cached) {
    walletId = cached.id; version = Number(cached.version||0); balance = cents(cached.balance_cents);
    $('cust').textContent = (cached.mobile || cached.name || 'Wallet') + ' • Balans ' + money(balance) + ' (kas)';
    renderCart();
  } else {
    $('cust').textContent = 'Laai…';
  }

  // then fetch fresh
  let w = null;
  try{
    const d = digits(raw);
    if (d && d.length >= 7 && d.length >= raw.length-2){
      const j = await fetch('/api/wallets/by-mobile/'+d).then(r=>r.json());
      if (j?.ok) w = j.wallet;
    }else{
      const j = await fetch('/api/wallets/'+encodeURIComponent(raw)).then(r=>r.json());
      if (j?.ok) w = j.wallet;
    }
  }catch(_e){}
  if (!w){ $('cust').textContent = 'Wallet nie gevind'; return; }

  walletId = w.id;
  version = Number(w.version||0);
  balance = cents(w.balance_cents);
  $('cust').textContent = (w.mobile || w.name || 'Wallet') + ' • Balans ' + money(balance);
  walletsLRUTouch(w);
  renderCart();
}

/* ------------------ flow ------------------- */
$('scanBtn').onclick = () => {
  const v = prompt('Voer/scan wallet ID of selfoon:');
  if (!v) return;
  $('wallet').value = v.trim();
  $('loadBtn').click();
};
$('loadBtn').onclick = async () => {
  const raw = ($('wallet').value||'').trim();
  if (!raw){ alert('Voer wallet ID of selfoon in'); return; }
  clearCart();
  await lookupWallet(raw);
};

$('checkoutBtn').onclick = async () => {
  const rows = Array.from(cart.values());
  if (!walletId || !rows.length) { alert('Leë mandjie.'); return; }
  const total = rows.reduce((s,r)=> s + r.qty*(r.price_cents||0), 0);

  // do NOT allow negative balances
  if (total > balance) { alert('Balans onvoldoende.'); return; }

  try{
    const r = await fetch('/api/wallets/'+encodeURIComponent(walletId)+'/deduct',{
      method:'POST', headers:{'content-type':'application/json'},
      body: JSON.stringify({
        items: rows.map(r => ({
          id: r.id, name: r.name, qty: r.qty, unit_price_cents: r.price_cents
        })),
        expected_version: version,
        bartender_id: 'bar-1',
        device_id: 'dev-1'
      })
    });
    const j = await r.json().catch(()=>({}));
    if (!r.ok || j.ok===false) throw new Error(j.error||'deduct_failed');
    balance = cents(j.new_balance_cents);
    version = Number(j.version||version);
    walletsLRUTouch({ id: walletId, balance_cents: balance, version, name: '', mobile: '' });
    clearCart();
    $('cust').textContent = 'Balans ' + money(balance);
  }catch(e){
    alert(e.message || 'Fout met afrekening.');
  }
};

$('clearBtn').onclick = clearCart;

/* init */
ensureItems();
</script>
</body></html>`;
}
