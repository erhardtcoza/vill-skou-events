...
$('startBtn').onclick = async () => {
  ...
  try {
    ...
    const txt = await r.text(); // tolerate non-JSON responses
    let j; try { j = JSON.parse(txt); } catch { j = { ok:false, error: txt?.slice(0,200) || 'bad response' }; }
    if (!j.ok) throw new Error(j.error || 'unknown');
    $('msg').textContent = \`Shift started (session #\${j.session_id}).\`;
    // next step (sell UI) will navigate here
  } catch (e) {
