...
<style>
  ...
  *{ box-sizing:border-box } body{ margin:0; font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif; background:var(--bg); color:#111 }
  ...
  h1{ margin:0 0 18px }
  .tabs{ display:flex; gap:14px; margin:0 0 16px; flex-wrap:wrap }
...
(function(){
  ...
  function money(cents){ const n = Number(cents||0)/100; return 'R' + n.toFixed(2); }
  ...
  async function getJSON(url){
    try{
      ...
      catch { return { ok:false, status:r.status, error:'Bad JSON', raw:txt }; }
    }catch(e){
  ...
  async function viewEvents(){
    ...
    if (!res.ok || !res.json || res.json.ok===false){
      ...
      err.textContent = 'Error loading events';
      wrap.textContent = '';
      return;
    }
    const rows = (res.json.events||[]).map(ev =>
      '<tr>'
      + '<td>'+esc(ev.id)+'</td>'
      + '<td>'+esc(ev.slug)+'</td>'
    ...
    wrap.innerHTML = rows
      ? '<table><thead><tr><th>ID</th><th>Slug</th><th>Name</th><th>Start</th><th>End</th><th>Status</th><th></th></tr></thead><tbody>'+rows+'</tbody></table>'
        + '<div id="ttBox" class="card" style="display:none; margin-top:14px"></div>'
      : '<div class="muted">No events.</div>';

    (wrap.querySelectorAll('button[data-tt]')||[]).forEach(btn=>{
      btn.onclick = async ()=>{
        const id = btn.getAttribute('data-tt');
        const box = $('#ttBox'); box.style.display='block'; box.textContent='Loading types…';
        const r = await getJSON('/api/admin/events/'+encodeURIComponent(id)+'/ticket_types');
        ...
        const lines = tts.map(tt =>
          '<tr>'
          + '<td>'+esc(tt.id)+'</td>'
          + '<td>'+esc(tt.name)+'</td>'
          + '<td>'+money(tt.price_cents)+'</td>'
        ...
        box.innerHTML = tts.length
          ? '<h3 style="margin:0 0 8px">Ticket types for event #'+esc(id)+'</h3>'
            + '<table><thead><tr><th>ID</th><th>Name</th><th>Price</th><th>Capacity</th><th>Per-order</th><th>Gender req</th></tr></thead>'
  ...
  async function viewTickets(){
    ...
    $('#tLoad').onclick = async ()=>{
      ...
      const rows = (s.by_type||[]).map(x =>
        '<tr>'
        + '<td>'+esc(x.name)+'</td>'
        + '<td class="right">'+(Number(x.price_cents||0)/100).toFixed(2)+'</td>'
        + '<td class="right">'+esc(x.total||0)+'</td>'
        + '<td class="right">'+esc(x.unused||0)+'</td>'
        + '<td class="right">'+esc(x.in||0)+'</td>'
        + '<td class="right">'+esc(x.out||0)+'</td>'
        + '<td class="right">'+esc(x.void||0)+'</td>'
        + '</tr>'
      ).join('');
      $('#tWrap').innerHTML = ''
        ...
        + '</div>'
        + '<table><thead><tr>'
        + '<th>Type</th><th class="right">Price (R)</th><th class="right">Total</th><th class="right">Unused</th>'
        + '<th class="right">In</th><th class="right">Out</th><th class="right">Void</th>'
  ...
  async function viewPOS(){
    ...
    async function load(){
      ...
      if (!r.ok || !r.json || r.json.ok===false){
        ...
        $('#posMsg').textContent='Error loading sessions.';
        $('#posWrap').textContent='';
        return;
      }
      const rows = (r.json.sessions||[]).map(s=>{
        ...
        const cash = ('cash_cents' in s) ? money(s.cash_cents) : 'R0.00';
        const card = ('card_cents' in s) ? money(s.card_cents) : 'R0.00';
        return '<tr>'
          + '<td>'+esc(s.id)+'</td>'
      ...
      $('#posWrap').innerHTML = rows
        ? '<table><thead><tr><th>ID</th><th>Cashier</th><th>Gate</th><th>Opened</th><th>Closed</th><th>Closed by</th><th class="right">Cash (R)</th><th class="right">Card (R)</th></tr></thead><tbody>'+rows+'</tbody></table>'
  ...
  async function viewVendors(){
    ...
    $('#vLoad').onclick = async ()=>{
      ...
      $('#vWrap').innerHTML = rows
        ? '<table><thead><tr><th>ID</th><th>Name</th><th>Contact</th><th>Stand #</th><th class="right">Staff quota</th><th class="right">Vehicle quota</th></tr></thead><tbody>'+rows+'</tbody></table>'
        : '<div class="muted">No vendors.</div>';
    };
  }
  ...
  function viewSite(){
    setActive('site');
    pane.innerHTML = ''
      + '<div class="card"><h2 style="margin:0 0 10px">Site settings</h2><div class="muted">Coming soon.</div></div>';
  }

  /* -------- Router -------- */
  function route(){
    const h = (location.hash||'').replace('#','');
    if (h==='tickets') return viewTickets();
    if (h==='pos') return viewPOS();
    if (h==='vendors') return viewVendors();
    if (h==='users') return viewUsers();
    if (h==='site') return viewSite();
    return viewEvents();
  }
